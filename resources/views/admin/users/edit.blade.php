...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-navy-800 mt-2">Edit User: {{ $user->name }}</h1>
</div>
...
<div class="bg-white rounded-2xl border border-gray-100 p-8 max-w-2xl">
    <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-5">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-semibold text-navy-800 mb-1.5">Password Baru <span class="text-gray-400 font-normal">(opsional)</span></label>
                <input type="password" name="password"
                    class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-navy-500 focus:border-transparent transition-all"
                    placeholder="Kosongkan jika tidak ganti">
            </div>
            <div>
                <label class="block text-sm font-semibold text-navy-800 mb-1.5">Konfirmasi Password</label>
