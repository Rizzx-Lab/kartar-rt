...
- **2026-07-22 — Public gallery display fixes (autoplay + auto-scroll regression)**
  - Two issues found and fixed in `frontend/components/ui/image-gallery.tsx`:
    ...
    2. **Auto-scroll regression (right photo column frozen when video active):** The right-side photo column in the video-active 3-column layout had two compounding bugs introduced when Task 6 was implemented:
       ...
       - **Admin scroll speed slider has never worked (root cause + fix):** The admin's "Kecepatan Scroll Galeri" slider in Pengaturan had no effect on either the video-active or non-video layout — it was broken from the start. Root cause: the CSS animation classes (`.animate-scroll-gallery-up`, `-slow`, `-medium`) in `globals.css` declare hardcoded durations directly (`30s`, `40s`, `35s`) and never read any CSS custom property or dynamic value. Meanwhile, `image-gallery.tsx` computed `videoScrollSpeed` from the admin setting but never applied it. The initial fix attempt (`--scroll-duration` CSS custom property) was also dead — the CSS never referenced that variable. Final fix: replaced all hardcoded animation class declarations with a direct inline `style={{ animationDuration: `${scrollDuration}s` }}` on every animated column `<div>` in both layout branches (6 columns total: left photo column + right photo column in video-active layout; left/center/right photo columns in non-video layout). `scrollDuration = Math.max(20, Math.min(scrollSpeed, 60))` is now applied directly as the `animation-duration` CSS property, which overrides whatever the Tailwind animation classes declare. Additionally: renamed `videoScrollSpeed` → `scrollDuration` for accuracy (it's a duration, not a speed); the `normalClass`/`slowClass`/`mediumClass` class names are kept only for `animation-name` + `linear` + `infinite`; the dead `--scroll-duration` custom property usage is removed.
  - **Admin sublabel corrected:** The sublabel below the slider previously said "Dalam detik (30 = normal, 60 = lambat, **15 = cepat**)" — but the code clamps to a minimum of 20, making 15 unachievable. Updated to "Dalam detik (20 = cepat, 30 = normal, 60 = lambat)" to match the actual clamp bounds.
